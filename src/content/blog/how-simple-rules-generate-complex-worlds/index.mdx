...
I learned about the Game of Life from [The Coding Train](https://youtu.be/FWSR_7kZuYg?si=tkyfWgT1C1k3kJNZ) about 6 years ago. Since then, I fell in love with these concepts and started exploring them further.
...
Let’s replace the Sudoku grid with a game's tileset. The rule is simpler now. For each tile, we have a set of possible neighbors that can be placed next to it. The constraints we are going to use are that the colors on adjacent tiles must match up with each other. Check out [Wave by Oskar Stålberg](https://oskarstalberg.com/game/wave/wave.html) to see the algorithm in action.
