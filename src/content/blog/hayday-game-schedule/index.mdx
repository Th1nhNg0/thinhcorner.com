...
<YouTubeEmbed
  id="mSev84nF1HA"
  title="Hay Day official trailer"
  className="mx-auto"
/>
...
We have a set of tasks $T=\{1,2,...,n\}$ that need to be scheduled on a set of machines $M=\{1,2,…,m\}$. Each task $t \in T$ has a duration $d t$ and can be assigned to one or more machines. Additionally, tasks may have dependencies such that a task $t$ can only start after another task $u$ finishes.
...
- If task $t$ depends on task $u$, then $S_t \geq E_u$.
