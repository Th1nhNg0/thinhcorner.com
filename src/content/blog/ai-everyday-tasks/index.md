...
For big tasks with heavy context, I also use subagents. Most models only work well within a ~256k token context window, so instead of cramming a huge project into one giant session, I spawn subagents — each one handles a small, focused task with a small context — and they report back to the main agent. Everything stays sharp, fast, and cheap.
