...
The crossover for this problem is a little bit complex. According to [this paper](https://www.researchgate.net/publication/281545095_SOLVING_JOB_SHOP_SCHEDULING_PROBLEM_WITH_GENETIC_ALGORITHM), they have used order crossover for tasks shuffling between two parent schedules. Whereas, uniform crossover operator is used for shuffling of machines assignment to tasks.
...
Uniform crossover is a type of crossover operator that works by randomly selecting genes from the parents to create the offspring. In uniform crossover, each gene in the offspring is selected from one of the parents with a certain probability.

As the author in the paper mentioned, they have used uniform crossover for shuffling of machines assignment to task.

When the same task in two parents have different machine assignment, the machine assignment of the task in the offspring is randomly selected from one of the parents. Here is the example of the uniform crossover:

<div className="flex justify-center items-center gap-10">
$
\text{ Parent1 }= \begin{bmatrix}
1 & 3 & 2 & 1 & 1 \\
4 & 5 & 2 & 3 & 1
\end{bmatrix}
$
